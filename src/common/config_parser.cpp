...
//       "Display label counts based on i-th input stream (-1 is last)", -1);
  ...
  cli.add<std::vector<std::string>>("--logical-epoch",
      ...
      {"1e", "0"});
  cli.add<std::string/*SchedulerFrom*/>("--save-from",
      "Save model starting from arg updates (append 't' for every  arg  target labels)",
      "10000u");
...
void ConfigParser::addOptionsValidation(cli::CLIWrapper& cli) {
  ...
  cli.add<size_t>("--early-stopping",
     ...
     10);
  cli.add<std::string/*SchedulerFrom*/>("--valid-from",
      "Validate model starting from arg updates (append 't' for every  arg  target labels)",
      "10000u");
